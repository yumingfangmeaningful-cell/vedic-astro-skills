...
## 🔍 外部检索（OmniSeek + Web 双开）

**参考：resources/external_research.md**

Q&A 阶段用户会问现实世界的问题（"这个专业现在就业怎么样""这个政策什么时候落地""要不要去某个城市"）。
这类问题里的**事实部分禁止用训练记忆回答**：

1. 默认 **OmniSeek + web search 双开**核实，标注来源和访问日期
2. 两边冲突 → 以一手/官方源为准，并写明分歧
3. 查不到 → 写"未能核实"，不许拿记忆补
4. 盘面结论不因检索结果改变；检索补的是"现实世界"那一半
5. 查询词禁止夹带用户姓名、完整出生信息或 user_context 内容

Step 1-3 盲审期间不做任何针对用户本人的检索。
...
13. **事实不凭记忆**：现实世界事实（行业/政策/时区/文献原文）必须 OmniSeek + web search 双开核实，标来源和日期；盘面数据仍以calculator为唯一来源。见resources/external_research.md
