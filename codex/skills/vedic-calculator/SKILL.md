...
> ⚠️ **上表只是常用城市速查值，不是可信来源。以下情形必须外部核实后再填，禁止凭记忆：**
> - **夏令时**：出生日期落在可能实行夏令时的年份/地区（如中国 1986~1991、台湾早年、欧美各国各自规则）
>   → 先核实"该地该年是否执行夏令时、出生当天钟面相对 UTC 的实际偏移"
> - **历史时区变更**：1949 年前后、国界或行政区划变更、时区标准调整过的地区
> - **小地方**：县/乡镇不在速查表里 → 坐标必须查，不要猜小数位
> - **非中印地区**：时区历史通常比"全境统一"复杂得多
>
> 核实方式：**OmniSeek + web search 双开**，协议见 `vedic-core/resources/external_research.md`。
> 隐私红线：只查"某地某年的时区规则"这类公共事实，禁止把用户姓名或完整出生信息作为查询词。
>
> **时区错 1 小时 → Lagna 可能整体错一个星座 → 全盘结论作废。这是本 skill 最高频的致命错误。**
